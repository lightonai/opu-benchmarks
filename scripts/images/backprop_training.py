...
def parse_args():
    ...
    parser.add_argument("-lr", help='Learning rate. Defaults to 1e-6', type=float, default=0.000001)
...
def save_data(args, final_data):
    ...
    final_data_columns = ['optimizer', 'epoch', 'train loss', 'test loss', 'test accuracy',
                          'training time', "test time", "model size-linear", "model size-tot", 'date']
    csv_name = "{}_{}_backprop.csv".format(args.model_name, args.model_options)
...
def main(args):
    ...
    model.classifier = nn.Linear(output_size, len(train_loader.dataset.classes))
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        model, epoch_loss, epoch_acc = train_model(model, train_loader, criterion_backprop, optimizer,
                                                   device=args.device)
        ...
        print(
            'Train Loss {:.4f}\tTrain acc: {:3.2f}\tTrain RT = {:3.2f}\tTest Loss {:.4f}\tTest acc: {:3.2f}\tTest RT = {:3.2f}'
            .format(epoch_loss, epoch_acc, training_time, test_loss, test_acc, test_time))

        # Save data
        epoch_data = [args.optimizer_name, epoch, epoch_loss, test_loss, test_acc, training_time, test_time,
                      model_size_linear, model_size_tot, datetime.now()]
